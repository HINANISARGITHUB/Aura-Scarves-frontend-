        ...
        >
          ...
          <div className="flex items-center gap-6 mb-10 pb-10 border-b border-aura-light">
            <div className="w-20 h-20 rounded-full bg-aura-accent/20 flex items-center justify-center">
              <span className="font-display text-4xl text-aura-accent">
                {user?.name?.[0]?.toUpperCase()}
              </span>
            </div>
            <div>
              <h2 className="font-display text-3xl text-aura-dark font-light">
                {user?.name}
              </h2>
              <p className="font-body text-xs text-aura-muted tracking-wide mt-1">
          ...
          {msg && (
            <div className="mb-6 font-body text-xs text-aura-accent tracking-wide">
              {msg}
            </div>
          )}
          ...
          <div className="space-y-6">
            <div>
              <label className="font-body text-[10px] tracking-[0.25em] uppercase text-aura-muted block mb-2">
                Full Name
              </label>
              {editing ? (
                <input
                ...
                <div className="flex items-center justify-between py-3 border-b border-aura-light">
                  <p className="font-body text-sm text-aura-dark">
                    {user?.name}
                  </p>
                  <button
            ...
            <div>
              ...
              <div className="py-3 border-b border-aura-light">
                <span
                  className={`inline-block px-3 py-1 font-body text-[10px] tracking-widest uppercase ${user?.role === "admin" ? "bg-aura-accent/15 text-aura-accent" : "bg-aura-bg text-aura-muted"}`}
                >
                  {user?.role}
                </span>
              </div>
          ...
          <div className="mt-10 flex gap-4">
            ...
            {!editing && (
              <button
                onClick={logout}
                className="btn-outline text-red-400 border-red-200 hover:bg-red-50 hover:border-red-300 hover:text-red-400"
              >
                Sign Out
              </button>
            )}
